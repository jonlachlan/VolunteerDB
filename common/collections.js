...
VolunteerJobs.allow({
  insert: function (userId, doc) {
    ...
    return (userId && Roles.userIsInRole(userId, ['organizer', 'manager']));
  },
  update: function (userId, doc, fields, modifier) {
    // can only change your own documents
    return (userId && (doc.owner === userId || Roles.userIsInRole(userId, ['organizer', 'manager'])));
  },
  remove: function (userId, doc) {
    // can only remove your own documents
    return (userId && (doc.owner === userId || Roles.userIsInRole(userId, ['manager'])));
  },
  fetch: ['owner']
});
